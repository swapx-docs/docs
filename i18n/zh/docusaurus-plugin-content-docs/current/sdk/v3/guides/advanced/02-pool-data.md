...
SwapX V3 allows 4 different Fee tiers when deploying a pool, so multiple pools can exist for each pair of tokens.
...
To construct the Contract we need to provide the address of the contract, its ABI and a provider connected to an [RPC endpoint](https://www.chainnodes.org/docs). We get access to the contract's ABI through the `@swapx/v3-core` package, which holds the core smart contracts of the SwapX V3 protocol:
...
The [slot0 function](contracts/v3/reference/core/interfaces/pool/ISwapXV3PoolState.md#slot0) represents the first (0th) storage slot of the pool and exposes multiple useful values in a single function:
...
If you are not familiar with the concept of ticks, check out the [`introduction`](01-introduction).
...
As mentioned, SwapX V3 Pools store **bitmaps**, also called _words_, that represent the state of **256 initializable ticks** at a time.
...
All ticks of SwapX V3 pools are between the indices `-887272` and `887272`.
...
A great visualization of what the bitMaps look like can be found in the [SwapX V3 development book](https://SwapXv3book.com/docs/milestone_2/tick-bitmap-index/):

<img 
  src="/img/tickBitmap_cut.png" 
  alt="TickBitmap" 
/>

We encourage anyone trying to get a deeper understanding of the SwapX protocol to read the SwapX V3 Book.
...
Now that you are familiar with fetching Pool data, continue your journey with the [next example](03-active-liquidity) on visualizing the Liquidity density of a pool.
