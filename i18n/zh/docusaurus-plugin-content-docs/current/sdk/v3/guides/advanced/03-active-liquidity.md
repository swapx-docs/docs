...
<img 
  src="/img/liquidityNetComparison.png" 
  alt="LiquidityNet1" 
/>
...
To fetch all ticks of our Pool, we will use the [SwapX V3 graph](api/subgraph/overview).
...
The active liqudity at the current Price is also stored in the smart contract - we already fetched it with the `liquidity` function in the [previous guide](02-pool-data).
...
```typescript
import { tickToPrice } from "@swapx/v3-sdk";
...
```typescript
import { TickMath, tickToPrice } from '@swapx/v3-sdk'
...
```typescript
const allProcessedTicks = previousTicks
...
Now that you are familiar with liquidity data, consider checking out our [next guide](04-price-oracle) on using SwapX as a Price Oracle.
